...
interface CardFanCarouselProps {
  cards: CardItem[];
  /** Tailwind classes controlling the size of each card slot */
...
export default function CardFanCarousel({ cards, cardClassName }: CardFanCarouselProps) {
  ...
  return (
    <div className="flex flex-col items-center w-full py-4 lg:py-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const wrapper = (
              <div className={`relative w-full h-full ${cardClassName ?? ""}`}>{card.content}</div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} className="fan-card block cursor-pointer">{wrapper}</a>
            ) : (
              <div key={index} className="fan-card">{wrapper}</div>
            );
          })}
        </div>
      </div>
